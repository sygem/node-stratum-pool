...
exports.createGeneration = function (rpcData, 
                                     blockReward, 
                                     feeReward, 
                                     recipients, 
                                     poolAddress, 
    ...
    if(!coinbase) coinbase = 'zellabs.net';
    ...
    if ((masternodePayment === false || masternodePayment === undefined) && payZelNodeRewards === false && !rpcData.coinbase_required_outputs) {
        ...
        var fluxCreationAddrHash = fluxCreationAddress ? bitcoin.address.fromBase58Check(fluxCreationAddress).hash : null;
        if (fluxCreationAddrHash != null) {
            txb.addOutput(
                scriptCompile(fluxCreationAddrHash),
                Math.round(fluxCreationAmount)
            );
        }
    }
